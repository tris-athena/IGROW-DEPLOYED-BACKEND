...
exports.CreateData = async (req, res) => {
  try {
      ...
      const { WaterSupply, WaterSource, AmountUsed } = req.body;
      const phTime = moment().tz("Asia/Manila").format("MMMM D, YYYY h:mm A");

      // Ensure required fields exist
      if (!WaterSupply || !WaterSource || !AmountUsed) {
          return res.status(400).json({ message: "Missing required fields" });
      }

      // Create and save new data
      const newData = new WaterSupplyModel({ WaterSupply, WaterSource, AmountUsed, createdAt: phTime });
      await newData.save();

...
const { CreateData } = require("../controllers/waterCycleController");
const WaterCycle = require("../models/watercycle");

// POST route to create a new water collection record
router.post("/wcl-create", CreateData);

// GET route to fetch all water collection records
router.get("/water-cycle", async (req, res) => {
  try {
    const records = await WaterCycle.find();
...
router.delete("/water-cycle/:id", async (req, res) => {
  try {
    const { id } = req.params; // Get the ID from the request parameters
    const deletedRecord = await WaterCycle.findByIdAndDelete(id); // Attempt to delete the record by ID

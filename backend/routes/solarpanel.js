...
const { CreateData } = require("../controllers/solarPanelController");
const SolarPanel = require("../models/solarpanel");

// POST route to create a new water collection record
router.post("/sp-create", CreateData);

// GET route to fetch all water collection records
router.get("/solar-panel", async (req, res) => {
  try {
    const records = await SolarPanel.find();
...
router.delete("/solar-panel/:id", async (req, res) => {
  try {
    const { id } = req.params; // Get the ID from the request parameters
    const deletedRecord = await SolarPanel.findByIdAndDelete(id); // Attempt to delete the record by ID

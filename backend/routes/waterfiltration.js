...
const { CreateData } = require("../controllers/waterFiltrationController");
const WaterFiltration = require("../models/waterfiltration");

// POST route to create a new water collection record
router.post("/wf-create", CreateData);

// GET route to fetch all water collection records
router.get("/water-filtration", async (req, res) => {
  try {
    const records = await WaterFiltration.find();
...
router.delete("/water-filtration/:id", async (req, res) => {
  try {
    const { id } = req.params; // Get the ID from the request parameters
    const deletedRecord = await WaterFiltration.findByIdAndDelete(id); // Attempt to delete the record by ID

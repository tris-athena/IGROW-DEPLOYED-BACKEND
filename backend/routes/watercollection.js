...
router.get("/water-collections", async (req, res) => {
  try {
...
router.delete("/water-collections/:id", async (req, res) => {
  try {
    const { id } = req.params; // Get the ID from the request parameters
    const deletedRecord = await WaterCollection.findByIdAndDelete(id); // Attempt to delete the record by ID

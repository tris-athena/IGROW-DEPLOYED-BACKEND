...
const { CreateData } = require("../controllers/waterTankQualityController");
const WaterTankQuality = require("../models/watertankquality");

// POST route to create a new water collection record
router.post("/wtq-create", CreateData);

// GET route to fetch all water collection records
router.get("/water-tank-quality", async (req, res) => {
  try {
    const records = await WaterTankQuality.find();
...
router.delete("/water-tank-quality/:id", async (req, res) => {
  try {
    const { id } = req.params; // Get the ID from the request parameters
    const deletedRecord = await WaterTankQuality.findByIdAndDelete(id); // Attempt to delete the record by ID
